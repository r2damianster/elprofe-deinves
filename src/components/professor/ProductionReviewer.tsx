...
export default function ProductionReviewer() {
  ...
  async function submitReview(productionId: string) {
    ...
    try {
      ...
      setEditing(prev => { const n = new Set(prev); n.delete(productionId); return n; });
      setExpanded(null);
            ...
            <div key={p.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              ...
              {expanded === p.id && (
                <div className="border-t border-gray-100 p-5 space-y-5">
                  ...
                  {p.status !== 'reviewed' || editing.has(p.id) ? (
                    <div className="space-y-3">
                      <h4 className="text-sm font-bold text-gray-700 flex items-center gap-2">
                        <AlertCircle className="w-4 h-4 text-blue-500" />
                        {editing.has(p.id) ? 'Editar calificación' : 'Calificar producción'}
                      </h4>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => submitReview(p.id)} disabled={submitting === p.id}
                          ...
                          {submitting === p.id ? 'Guardando...' : editing.has(p.id) ? 'Actualizar calificación' : 'Enviar calificación'}
                        </button>
                        {editing.has(p.id) && (
                          <button onClick={() => setEditing(prev => { const n = new Set(prev); n.delete(p.id); return n; })}
                            className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm hover:bg-gray-300 transition">
                            Cancelar
                          </button>
                        )}
                    ...
                    <div className="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-xl">
                      <div>
                        <p className="text-sm font-bold text-green-800">Calificación: {p.score}/10</p>
                        {p.feedback && <p className="text-xs text-green-700 mt-1">{p.feedback}</p>}
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <button
                          onClick={() => {
                            setScores(prev => ({ ...prev, [p.id]: String(p.score ?? '') }));
                            setFeedbacks(prev => ({ ...prev, [p.id]: p.feedback ?? '' }));
                            setEditing(prev => new Set(prev).add(p.id));
                          }}
                          className="text-xs px-3 py-1.5 border border-blue-400 text-blue-600 rounded-lg hover:bg-blue-50 transition font-medium">
                          Editar
                        </button>
                        {p.attempts < 2 && (
